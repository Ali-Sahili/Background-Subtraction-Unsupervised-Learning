...
struct SSContext
{
    ...
    BackgroundSubtractorSuBSENSE* subtractor;
    cv::Size2i imgSize;
};
...
void ss_apply(SSContext* ctx, void* img_data, void* output, void* BgOutput)
{
    ...
    cv::medianBlur(output_img,output_img,5);
    cv::medianBlur(output_img,output_img,5);

    cv::Mat output_bg(ctx->imgSize, CV_8UC1, BgOutput);
    ctx->subtractor->getBackgroundImage(output_bg);
}
